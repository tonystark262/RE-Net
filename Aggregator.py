...
class MeanAggregator(nn.Module):
    ...
    def predict(self, s_history, s, r, ent_embeds, rel_embeds):
        ...
        for i, s_o in enumerate(s_history):
            ...
            inp[i] = torch.cat((tem, ent_embeds[s], rel_embeds[r]), dim=0)
        return inp
...
class RGCNAggregator(nn.Module):
    ...
    def forward(self, s_hist, s, r, ent_embeds, rel_embeds, graph_dict,
                reverse):
        s_len_non_zero, s_tem, r_tem, g, node_ids_graph = get_sorted_s_r_embed_rgcn(
            s_hist, s, r, ent_embeds, graph_dict)
        if g is None:
            s_packed_input = None
        else:
    ...
    def predict(self, s_history, s, r, ent_embeds, rel_embeds, graph_dict,
                reverse):
        s_hist = s_history[0]

        s_hist_t = s_history[1]
        s_len_non_zero, s_tem, r_tem, g, node_ids_graph = get_sorted_s_r_embed_rgcn(
            ([s_hist], [s_hist_t]), s.view(-1, 1), r.view(-1, 1), ent_embeds,
            graph_dict)
        self.rgcn1(g, reverse)
        self.rgcn2(g, reverse)
